...
def gen_table(df, doc):
    ...
    '''
    color = 'white'
    with doc.create(pl.Tabular('l | c c c c', col_space='0.15cm', pos=['h'])) as table:
...
if __name__ =='__main__':
    ...
    df = pd.read_excel("./data/indicators_input.xlsx", sheetname=dc,
                       header=0, index_col=0, usecols=[0, 1, 2, 3, 4, 5])
    df.iloc[:,0] = df.iloc[:,0].map(lambda x: np.round(x/1000.0, 1))
    df.iloc[:,1] = df.iloc[:,1].map(lambda x: np.round(x/1000.0, 1))
    df.iloc[:,2] = df.iloc[:,2].map(lambda x: np.round(x/1000.0, 1))
    df.iloc[:,3] = df.iloc[:,3].map(lambda x: np.round(x/1000.0, 1))
    doc = gen_doc(dc)
    gen_table(df, doc)
    doc.generate_tex()
    print("Open interest by Contracts is done!")
